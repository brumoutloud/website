...
function getOpeningStatus(openingHoursText) {
    ...
    const now = new Date();
    // Using 'en-GB' locale for toLocaleString can sometimes cause timezone issues if server isn't in London.
    // Better to use UTC methods or a specific timezone library if precise timezone handling is critical.
    // For this context, assuming BST/GMT is handled by the environment's default or this approximation works.
    ...
    const checkStatus = () => {
        ...
        for (const slot of (schedule[currentDayIndex] || [])) {
            ...
            } else { // Overnight slot: starts today, ends tomorrow (e.g., 7pm - 2am)
                isOpenNow = (currentTimeInMinutes >= slot.opens || currentTimeInMinutes < slot.closes);
            }
