...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    // If the slug does NOT contain a date, it's a parent URL.
    // We need to find the next instance and redirect to it.
    if (!dateMatch) {
        // We can't just convert the slug back to a name, as names can have characters slugs don't.
        // Instead, we find the first event with this slug to get its Parent Event Name.
        const firstRecord = await base('Events').select({
            maxRecords: 1,
            filterByFormula: `OR({Slug} = "${slug}", STARTS_WITH({Slug}, "${slug}-"))`
        }).firstPage();

        if (!firstRecord || firstRecord.length === 0) {
            return { statusCode: 404, body: 'Event series not found.' };
        }
        
        const parentName = firstRecord[0].fields['Parent Event Name'] || firstRecord[0].fields['Event Name'];

        const nextInstanceRecords = await base('Events').select({
            maxRecords: 1,
            // Find the next event in the series that is happening today or in the future
            filterByFormula: `AND({Parent Event Name} = "${parentName.replace(/"/g, '\\"')}", IS_AFTER({Date}, DATEADD(TODAY(), -1, 'days')))`,
            sort: [{ field: 'Date', direction: 'asc' }]
        }).firstPage();

        // If we found an upcoming instance, redirect the user to its unique slug.
        ...
        // If no upcoming instance is found, show an error.
        return { statusCode: 404, body: 'This event series has concluded or has no upcoming dates.' };
    }
    
    // If the slug DOES contain a date, fetch that specific event instance.
    ...
    // --- The rest of the page generation logic from your live code ---
