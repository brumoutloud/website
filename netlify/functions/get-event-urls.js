...
async function getRenderedHtml(url) {
    ...
    // Increased timeout for the scraper API itself
...
// This function asks Gemini to parse the HTML and find the links.
async function parseUrlsFromHtmlWithAI(html) {
    ...
    const prompt = `From the following HTML, extract the href attribute from every link that points to an event detail page (they usually start with '/event/'). Return the data as a JSON array of strings. For example: ["/event/slug-1", "/event/slug-2"]. HTML: ${html}`;
...
exports.handler = async function (event, context) {
    // Netlify's Pro plan has a longer timeout. This increases it if available.
