...
exports.handler = async function (event, context) {
    ...
    try {
        ...
        // Using precise selectors based on the HTML you provided.
        const eventName = $('h1.eventitem-title').text().trim();

        if (eventName) {
            // **FIX 1: More robust date and time scraping**
            let eventDateStr, eventTimeStr;
            // First, try the multi-day format
            const multiDayDate = $('.eventitem-meta-date time.event-date').first();
            if (multiDayDate.length) {
                eventDateStr = multiDayDate.attr('datetime');
                eventTimeStr = $('.eventitem-meta-time time.event-time-localized').first().text().trim();
            } else {
                // Fallback for single day events
                eventDateStr = $('.eventlist-meta-date time.event-date').attr('datetime');
                eventTimeStr = $('.eventlist-meta-time time.event-time-localized-start').text().trim();
            }
